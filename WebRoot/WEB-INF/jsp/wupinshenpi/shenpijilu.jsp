...
<body>
<table id="table-shenpijilu-geren">
    <tbody>
    <s:iterator value="goodsformsList">
        <tr>
            <td>${gapplicant.name}</td><td>${applytime}</td><td>${muchGoods}</td><td>${donetime}</td><td>${reason}</td><td>${status}</td><td>叶文课</td>
        </tr>
      </s:iterator>
    </tbody>    
</table>
